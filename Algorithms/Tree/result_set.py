...
class KNNResultSet:
    def __init__(self, capacity):#capacity是等于k，在main函数声明了
    ...
    def worstDist(self):#因为worst_dist始终在变，所以需要这个函数的存在。
    ...
    def add_point(self, dist, index):
        ...
        if self.count < self.capacity:
            self.count += 1 #count的范围是1~5
     
        i = self.count - 1 #i的范围是0~4
  
        while i > 0:
            if self.dist_index_list[i - 1].distance > dist:
                # print('####')
                # print(self.dist_index_list[i - 1].distance)
        ...
        self.dist_index_list[i].index = index#范围是0~7，应该是做了8次比较
    ...
    def __str__(self):
        ...
        for i, dist_index in enumerate(self.dist_index_list):
            output += '%d - %.2f\n' % (dist_index.index, dist_index.distance)#打印每行index和distance
